...
// ── Locality options for new user signup ──
const LOCALITIES = [
...
export default function LoginScreen() {
  ...
  // UI state
  const [mode, setMode]         = useState('welcome') // 'welcome' | 'phone' | 'otp' | 'signup' | 'email' | 'demo'
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (!profile || !profile.name || profile.name === 'Local Resident') {
        // New user — collect name + locality
        setIsNewUser(true)
        setMode('signup')
      } else {
        navigate('/home')
  ...
  // ── SUPABASE: Complete new user signup ──
  ...
  // ── SUPABASE: Email login (dev/testing fallback) ──
  ...
  // ── DEMO: Instant login with preset user ──
      ...
      <div className="login-screen">
        ...
        <div className="login-hero">
          ...
          <div className="login-tagline">
            Real help and updates from people near you.{'\n'}
            Kharghar se shuru karo. 🏘️
          </div>
          {isSupabaseConfigured && (
            <div style={{ marginTop: 12, fontSize: 11, color: 'rgba(255,255,255,0.4)', display: 'flex', alignItems: 'center', gap: 4 }}>
              🔒 Secured by Supabase
            </div>
          )}
        </div>
        ...
        <div className="login-form-area">
          ...
          {mode === 'welcome' && (
            <>
              ...
              {isSupabaseConfigured ? (
                <>
                  <button className="btn btn-primary" onClick={() => { clearError(); setMode('phone') }}>
                    📱 Login / Sign up with Phone
                  </button>
                  <div className="divider">or</div>
                  <button className="btn btn-secondary" onClick={() => { clearError(); setMode('email') }}>
                    📧 Login with Email (for testing)
                  </button>
                  <div className="divider">or</div>
                  <button
                    style={{ fontSize: 13, color: 'var(--text-muted)', textAlign: 'center', padding: 8 }}
                    onClick={() => { clearError(); setMode('demo') }}
                  >
                ...
                <>
                  {/* No Supabase — show demo mode prominently */}
                  <div style={{ background: 'var(--warning-light)', borderRadius: 'var(--radius-sm)', padding: '10px 14px', fontSize: 13, color: '#78350F', marginBottom: 4 }}>
                    ⚙️ <strong>Demo Mode</strong> — Supabase not configured yet. Running with local data.
                  </div>
                  <button className="btn btn-primary" onClick={() => { clearError(); setMode('demo') }}>
                    🚀 Enter as Demo User
                  </button>
                  <div className="divider">or</div>
                  <button className="btn btn-secondary" onClick={() => { clearError(); setMode('phone') }}>
                    📱 Login with Phone (preview)
                  </button>
                </>
              )}
            </>
          ...
          {mode === 'phone' && (
            <>
              <button onClick={() => setMode('welcome')} style={{ color: 'var(--primary)', fontWeight: 700, fontSize: 14, textAlign: 'left' }}>
                ← Back
              </button>
              <div className="login-title">Enter your mobile number</div>
              <div className="login-sub">We'll send a one-time password via SMS.</div>
              ...
              <button
                ...
                onClick={isSupabaseConfigured ? handleSendOtp : () => { setMode('otp') }}
          ...
          {mode === 'otp' && (
            <>
              <button onClick={() => setMode('phone')} style={{ color: 'var(--primary)', fontWeight: 700, fontSize: 14 }}>
                ← Back
              </button>
              <div className="login-title">Enter OTP</div>
              <div className="login-sub" style={{ lineHeight: 1.5 }}>
                Sent to +91 {phone}.{' '}
                <button onClick={() => setMode('phone')} style={{ color: 'var(--primary)', fontWeight: 600 }}>
                  Change number
                </button>
              </div>
              ...
              <div className="form-group">
                <label className="form-label">
                  {isSupabaseConfigured ? 'OTP from SMS' : 'OTP (enter any 4 digits for demo)'}
                </label>
                <input
                  className="form-input"
                  type="tel"
              ...
              <button
                ...
                onClick={isSupabaseConfigured ? handleVerifyOtp : () => {
                  // Demo fallback: create a dummy user
                  const demoUser = {
                    id: 'user_' + Date.now(),
                    name: 'Local Resident',
                    phone,
              ...
              <button
                style={{ fontSize: 13, color: 'var(--primary)', textAlign: 'center', padding: 8, fontWeight: 600 }}
                onClick={isSupabaseConfigured ? handleSendOtp : () => {}}
                disabled={loading}
              >
          ...
          {mode === 'signup' && (
            <>
              ...
              >
                {loading ? '⏳ Saving...' : '🏘️ Join LocalSetu →'}
              </button>
            </>
          )}

          {/* ── Email Login (dev/testing only) ── */}
          {mode === 'email' && (
            <>
              <button onClick={() => setMode('welcome')} style={{ color: 'var(--primary)', fontWeight: 700, fontSize: 14 }}>
                ← Back
              </button>
              ...
              <div style={{ background: 'var(--info-light)', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: 'var(--info)', marginBottom: 4 }}>
                ℹ️ For development and testing only. Production users should use phone OTP.
              </div>

              <div className="form-group">
                <label className="form-label">Email</label>
                <input className="form-input" type="email" placeholder="you@example.com" value={email} onChange={e => { setEmail(e.target.value); clearError() }} />
              </div>
              <div className="form-group">
                <label className="form-label">Password</label>
                <input className="form-input" type="password" placeholder="Password" value={password} onChange={e => { setPassword(e.target.value); clearError() }} />
              </div>
            ...
            <>
              ...
              <div className="demo-login-options">
                ...
                <button
                  ...
                  onClick={() => handleDemoLogin(DEMO_USERS.find(u => u.role === 'admin'))}
